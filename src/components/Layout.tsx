    ...
    <div className="min-h-screen flex flex-col bg-paper">
        ...
        <div className="marquee-track flex whitespace-nowrap gap-12 font-mono text-xs tracking-widest uppercase">
          {Array.from({ length: 2 }).map((_, i) => (
            <div key={i} className="flex gap-12">
              ...
              <span>✦ Transparan &amp; aman sejak 2026</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <header className="border-b-2 border-ink bg-paper sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-4 flex items-center justify-between">
          <Link to={currentUser ? (currentUser.role === 'admin' ? '/admin' : '/member') : '/'} className="flex items-center gap-3">
            ...
            <div>
              <div className="font-display text-2xl font-bold leading-none">Cuan Dari Gratisan</div>
              <div className="font-mono text-[10px] tracking-widest uppercase text-ink-soft">Klik Iklan Dapat Cuan</div>
            </div>
          </Link>
      ...
      <footer className="border-t-2 border-ink bg-ink text-paper mt-16">
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-10 grid md:grid-cols-3 gap-8">
          <div>
            <div className="font-display text-2xl font-bold">Cuan Dari Gratisan</div>
            <p className="text-sm text-paper/70 mt-2 leading-relaxed">
              Klik iklan dapat cuan dan anda juga bisa mempromosikan bisnis anda.
            </p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="mt-3 text-sm text-paper/70">naufinofficials@gmail.com<br/>Jl. Setiabudi Psr III Kel. Medan Selayang, Sumatera Utara</p>
            <div className="mt-3 font-mono text-[10px] text-paper/50">© {new Date().getFullYear()} Cuan Dari Gratisan · Klik Iklan Dapat Cuan</div>
          </div>
        </div>
      </footer>
    </div>
